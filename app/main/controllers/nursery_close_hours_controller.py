...
def create_nursery_close_hour(close_hour: schemas.NurseryCloseHourCreate, db: Session = Depends(get_db)):
    try:
        return crud.NurseryCloseHourCRUD.create_nursery_close_hour(db, close_hour)
...
def read_nursery_close_hour(close_hour_uuid: str, db: Session = Depends(get_db)):
    db_close_hour = crud.NurseryCloseHourCRUD.get_nursery_close_hour(db, close_hour_uuid)
...
def update_nursery_close_hour(close_hour_uuid: str, close_hour: schemas.NurseryCloseHourUpdate, db: Session = Depends(get_db)):
    updated_close_hour = crud.NurseryCloseHourCRUD.update_nursery_close_hour(db, close_hour_uuid, close_hour)
    if updated_close_hour is None:
        raise HTTPException(status_code=404, detail="Nursery close hour not found")
    return updated_close_hour
